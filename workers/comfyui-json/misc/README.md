...
Production templates must set **`VAST_WORKLOAD_UNITS_<LANE>`** for each `generation_lane` you serve. Requests must include either a known `generation_lane` (plus matching template env when `vast_workload_units` is omitted) or **`input.vast_workload_units`**. There is no global **`VAST_WORKLOAD_UNITS`** on the worker; invalid or incomplete workload inputs raise `ValueError`. The Text2Image benchmark fallback sets `generation_lane` from **`BENCHMARK_GENERATION_LANE`** when possible. See **`comfy-vast-serverless/docs/VAST_BENCHMARK_LANES_AND_BOT_COST.md`** (monorepo root) if present.
