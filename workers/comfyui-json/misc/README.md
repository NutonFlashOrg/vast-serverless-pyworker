...
| `benchmark_LTX23_5090_AI2V.json` | `LTX23_5090`, `LTX23_5090_AI2V` | LTX **2.3** AI2V from prod `workflows_api/AI2V/LTX2.3_AI2V_Audio_api.json`. **96×144** intake (same as WAN I2V benchmarks via `INTConstant` nodes 292/293). Image → `LoadImage` (staged from S3); audio → `Load Audio` node (S3 via `BENCHMARK_AUDIO_*` + `BENCHMARK_IMAGE_*`). |
